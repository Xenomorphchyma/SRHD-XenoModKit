...
Python-ядро ModKit не имеет обязательных пакетных зависимостей. Внешние EXE нужны только для операций, использующих оригинальные игровые кодеки: полного `DAT ↔ TXT`, сборки `RSON/SVR → SCR` и преобразования `GI ↔ PNG`.
...
Нужен для сборки и конвертации RSON, SVR и SCR.
...
ModKit запускает старую GUI-subsystem программу на отдельном невидимом desktop, ждёт выходные файлы и завершает процесс. Видимый GUI для сборки не требуется.

## RangerTools для GI ↔ PNG

Проверенная локальная конфигурация использует:

```text
<TOOLS_ROOT>/RangerTools/gi-to-png_ranger-tools.exe
<TOOLS_ROOT>/RangerTools/png-to-gi_ranger-tools.exe
```

Контрольные суммы проверенных сборок:

- `gi-to-png_ranger-tools.exe`: `5329F2E113D524E186BC3B21766DBECB2D5E73517CF1C73A0CC2F24714498B25`
- `png-to-gi_ranger-tools.exe`: `48ECB9CCD4DA2CDE1AFB629F23ABB8571209A3F135C36520A30FEB19EB7F509B`

Авторитетный публичный канал распространения именно этих старых сборок подтвердить не удалось. Поэтому они не загружаются автоматически и не помещены в репозиторий. Без них недоступны только команды `convert gi-png` и `convert png-gi`; аудит сигнатур, побайтное сохранение GI и остальные форматы продолжают работать.

## Необязательные GUI-инструменты

ResEditor и ShipViewer не нужны headless-библиотеке и автоматическому выпуску модов. ModKit знает их только для явно разрешённой ручной команды `open`, которая по умолчанию заблокирована. Автоматический агент не должен устанавливать или запускать их.

Все права на сторонние инструменты принадлежат их авторам. SRHD XenoModKit фиксирует происхождение и хэши, но не присваивает их авторство.
