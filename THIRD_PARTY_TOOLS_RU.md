...
Python-ядро ModKit не имеет обязательных пакетных зависимостей. Внешние EXE нужны только для полного `DAT ↔ TXT`, декомпиляции `SCR → RSON` и сборки `RSON/SVR → SCR`. Преобразование `GI ↔ PNG` выполняет собственный Python-кодек ModKit.
