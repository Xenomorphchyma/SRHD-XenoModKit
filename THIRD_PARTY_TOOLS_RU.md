...
Python-ядро ModKit не имеет обязательных пакетных зависимостей. Внешние EXE нужны только для операций, использующих оригинальные игровые кодеки: полного `DAT ↔ TXT`, декомпиляции `SCR → RSON`, сборки `RSON/SVR → SCR` и преобразования `GI ↔ PNG`.
