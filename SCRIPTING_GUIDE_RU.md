...
кода и типов. По умолчанию каждый этап получает 60 секунд без подтверждённого
прогресса и общий аварийный потолок 300 секунд. Ожидаемый файл, файловый
ввод-вывод RScript и переход шага скрытой автоматизации сдвигают короткое окно;
одна лишь загрузка CPU его не сдвигает. Значение `0` у обоих параметров
полностью отключает ограничения.
...
действует 60-секундное скользящее окно без прогресса и общий аварийный потолок
300 секунд. Положительное значение `--timeout` заменяет общий потолок, а окно
без прогресса остаётся не длиннее 60 секунд.
