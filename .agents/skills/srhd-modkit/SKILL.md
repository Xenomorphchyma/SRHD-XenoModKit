...
3. Выполнить `python -B srhd.py --version` из `<MODKIT_ROOT>`. Ожидать `SRHD ModKit 0.8.0` или новее.
4. При другой версии сначала прочитать локальные `README_RU.md`, `AUDIT_RU.md` и вывод `python -B srhd.py --help`; не предполагать наличие команд 0.8.0.
...
- Для скриптов сначала выполнять `script audit-mod` и `script lint-runtime`; изменять RSON headless-командами и собирать через `script build`.
