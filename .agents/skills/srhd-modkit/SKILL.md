...
3. Выполнить `python -B srhd.py --version` из `<MODKIT_ROOT>`. Ожидать `SRHD ModKit 0.9.1` или новее.
...
- В скриптах не сохранять сырой `Item` в `TVar`/массив между ходами: сохранять `Id(item)` и восстанавливать текущий объект через `IdToItem`. Разгрузку, `OrderTakeOff` и `ShipOut` разделять границей хода; при `ShipOut` во время прямого обхода группы использовать обратный обход или корректировать индекс.
