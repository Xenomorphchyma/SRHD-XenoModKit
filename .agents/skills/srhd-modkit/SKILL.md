...
- В скриптах не сохранять сырые `Item`, `Planet`, `Star` или `Ship` в долгоживущем `TVar`/массиве: сохранять `Id(object)`, при миграции сначала обнулять старую ссылку и восстанавливать объект через соответствующий `IdTo*`. Разгрузку, `OrderTakeOff` и `ShipOut` разделять границей хода. Никогда не выполнять `ShipOut` текущего `GroupShip` в прямом цикле с повторным `GroupCount`, даже с уменьшением индекса; использовать отдельный обратный проход и завершать обработчик после изменения группы.
