...
3. Выполнить `python -B srhd.py --version` из `<MODKIT_ROOT>`. Ожидать `SRHD ModKit 0.9.2` или новее.
...
- В скриптах не сохранять сырые `Item`, `Planet`, `Star` или `Ship` в долгоживущем `TVar`/массиве: сохранять `Id(object)` и при миграции сначала обнулять старую ссылку. Для `Planet`/`Ship` использовать подтверждённые `IdToPlanet`/`IdToShip`. Не считать `IdToStar` встроенной функцией: звезду восстанавливать локальным ограниченным обходом `GalaxyStars()`/`GalaxyStar(i)` либо через сохранённую планету. Разгрузку, `OrderTakeOff` и `ShipOut` разделять границей хода. Никогда не выполнять `ShipOut` текущего `GroupShip` в прямом цикле с повторным `GroupCount`, даже с уменьшением индекса; использовать отдельный обратный проход и завершать обработчик после изменения группы. В динамических массивах после `newarray(1)`/`ArrayClear` начинать записи с `1`, пустоту проверять через `ArrayDim <= 1`, а парные persistent-массивы — на равенство размеров. Не передавать Item третьим аргументом `RndObject`, не менять `Order*`/`ShipSetBad` до проверки `ShipInHyperSpace` и не использовать цель `ShipGetBad` после разрыва систем без повторной пространственной проверки.
...
8. Сохранять штатное 60-секундное окно без подтверждённого прогресса и общий аварийный потолок 300 секунд. При доказанно долгой операции явно увеличить общий потолок; значение `0` отключает оба ограничения, но синтаксический preflight остаётся обязательным.
