...
Исправлять код сохранением `Id(object)` с последующим `IdTo*`, миграционным
обнулением старой ссылки, границей хода после `OrderTakeOff` и отдельным обратным
обходом изменяемой группы. Уменьшение индекса прямого цикла не считается
