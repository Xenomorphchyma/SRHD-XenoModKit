...
- Fixed-массивы проверяются на неизвестные читаемые слоты, выходы за `0..N-1` и ошибочное `<= ArrayDim(array)`. Прямые присваивания целого, float, строки, `CT` или `Format` типизируют слот сами и больше не требуют искусственного предварительного заполнения нулями.
...
- `runtime-hot-growing-membership-scan` предупреждает о верхней оценке `O(S³)`, когда два мировых множителя дополняются линейным поиском по растущему локальному массиву. Плоская точная матрица `S×S` остаётся допустимой.
- Стоимость мирового обхода в пользовательской функции переносится по call graph. Скрытый `S×S` показывается как info, третий мировой множитель — как warning; диагностика содержит вычисленный путь от `Turn` до helper. Эти предупреждения блокируются только в осознанном строгом режиме.
...
- `runtime-dialog-msg-eager-array-index` блокирует доказанный выход за границы в `<array[index]>` поля `TDialogMsg/TDialogAnswer.Msg`, которое вычисляется раньше связанного action-handler. Недоказанный диапазон остаётся warning; простая поздно вычисляемая подстановка вроде `<cost>` получает `runtime-dialog-msg-eager-mutable-value`. Безопасно либо подготовить одну строковую переменную до `DChange` и подставить её в `Msg`, либо оставить `Msg` пустым и сформировать всю реплику одним `DText`.
- `runtime-dialog-handler-dtext-overwrite` предупреждает о непустом `TDialogMsg.Msg`, связанном с handler, который вызывает `DText`: вызов заменяет всю реплику и способен удалить её начало и цветовую разметку.
- `runtime-modal-dialog-from-nextday` блокирует модальный `Dialog(...)`, достижимый из фонового `Code.Type=Turn`: синхронное окно внутри `TGalaxy.NextDay` способно повторно войти в AI или остановить смену дня. Turn должен только поставить pending-флаг; разговор показывается при следующем пользовательском контакте.
- `runtime-truce-state-reentry-cycle` блокирует `TruceBetweenShips`, достижимый из `TState` напрямую или через helper: функция немедленно запускает state-код обоих кораблей, поэтому простой `SetData`-guard не доказывает отсутствие вложенного цикла. Одноразовые диалоговые действия и независимый путь отхода без связи с `TState` остаются разрешены.
...
- `runtime-quest-item-image-layout-atypical` предупреждает о собственной предметной иконке, которая зарегистрирована правильно, но упакована не как типичный GI type 2 с тремя RLE-слоями. Это предупреждение о совместимости уменьшенных карточек, не запрет редких форматов. `resource info/verify --json` дополнительно сообщает bounds альфы, прозрачные поля, асимметрию и взвешенный центр без субъективной оценки рисунка.
...
- `quest info` содержит полную таблицу изображений по локациям и вариантам
  текста, списки пропущенных кадров и статистику повторного использования.
- Повтор одного кадра в шести и более разных локациях отмечается предупреждением
  с перечнем ID, но не объявляется ошибкой игрового формата.
- Новая релизная проверка `quest-media` сверяет QMM и изображения карточек с
  `Bm.PQI.*`, обнаруживает отсутствующие, незарегистрированные и неиспользуемые
  локальные файлы, читает размеры JPEG/PNG и цветовой режим без Pillow.
  Стандарт `343x394 RGB` проверяется наглядно и остаётся предупреждением.
- `quest-cards` проверяет согласованность ID, пути QM/QMM, `Length`, `Dif`,
  `hardness`, изображения, `ItemForPlanetQuest`, `StartText` и сообщает
  ожидаемое число песочных часов. Фактическое появление индикатора остаётся
  ручной проверкой интерфейса игры.
- `.py` получил штатный формат и проверку синтаксиса без выполнения кода.
  Корректные инструменты больше не создают `unsupported`, повреждённый Python
  блокирует релиз как `python-syntax-invalid`.
- Визуальная семантика персонажей и предметов намеренно не угадывается.
...
- `script compare-scr` сравнивает persistent-схему, смысловую карту диалогов и
  необязательные риски обновления SAV для изменившегося SCR с прежним
  runtime-именем.
...
В наборе из 242 тестов используются нативные PNG/GI/QM/QMM-кодеки и локальные BlockParEditor/RScript:
