...
в `D:\SRHD_Modding`, сохраняет неизвестные бинарные файлы без изменений и
...
| `.gai` | проверка заголовка/таблицы кадров, список, извлечение вложенных GI | собственный headless-инспектор |
| `.hai` | проверка заголовка, размеров и физической разметки всех кадров | собственный read-only-инспектор |
| `.pkg` | список файлов, полная проверка блоков ZL02/zlib, безопасная распаковка | собственный headless-декодер |
| `.scr` | версия, строки и фрагменты кода, сборка из RSON | собственный анализатор + RScript 4.10f CLI |
...
| `.wav`, `.psd`, `.jpg`, `.bmp`, `.vdo`, архивы, неизвестные | проверка известных сигнатур и точное копирование | passthrough |
...
`GAI` и `PKG` теперь разбираются собственной библиотекой без ResEditor. Для GAI
проверяются непрерывность таблицы и каждый вложенный GI; для PKG — каталоги,
имена, смещения, сжатые/исходные размеры и каждый блочный поток `ZL02`. `HAI`
пока read-only: заголовок и границы кадров проверяются, но запись отключена до
точного изучения пиксельных слоёв. Неизвестные файлы сохраняются без изменений.
...
`--overwrite`. Перед записью PKG полностью распаковывается и проверяется в памяти,
а имена очищаются от абсолютных путей и `..`. Пересборка GAI/HAI/PKG намеренно
не заявлена: сначала требуется доказать совместимость результата с игрой.
...
```python
from srhd_modkit import Toolchain, discover_mods, inspect_file, load_blockpar, stage_tree
from srhd_modkit.resources import inspect_gai, inspect_hai, inspect_pkg, extract_resource
...
66 тестов используют настоящие локальные RangerTools, BlockParEditor и RScript:
проверяют пиксель-в-пиксель круговой проход ARGB8888, ASCII/Unicode DAT,
события TState в собранном SCR, runtime-блокировки, граф RSON, GAI/HAI/PKG и
запрет GUI по умолчанию.
