...
- Поддерживаются оба реальных расположения: `DATA/Script/Lang.dat` и `CFG/<язык>/Lang.dat`. Все поставляемые переводы должны содержать ключ, но их текст не обязан совпадать.
