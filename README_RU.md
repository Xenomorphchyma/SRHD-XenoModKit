...
По умолчанию время RScript рассчитывается адаптивно по размеру файла, числу
объектов и строк кода. Формула не имеет верхнего потолка, поэтому крупный мод не
блокируется фиксированными 180/300 секундами. Для особого проекта можно передать
положительный `--timeout`, `--decompile-timeout` или `--roundtrip-timeout`; ноль
полностью отключает общий дедлайн. Статический preflight всё равно выполняется
до запуска старого компилятора.
