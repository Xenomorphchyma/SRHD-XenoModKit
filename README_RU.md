...
- ModKit определяет VERSIONINFO `RScript.exe` и поддерживает обе проверенные ветви CLI. RScript 4.15f стал штатным и декомпилирует SCR настоящей командой `--cli -d`; 4.10f остаётся понятным legacy-бэкендом без видимого GUI.
- Добавлены модульные RSM: `script export-rsm`, `script validate-rsm`, `script build-rsm`. Импортируемые файлы проверяются и фиксируются SHA-256; выход rsmc проходит SCR round-trip, runtime-lint и языковой аудит до публикации.
- Для `rsmc --lang-txt/--lang-dat` ModKit заранее создаёт существующий BlockPar-файл и обязательный узел `Script/<ScriptName>`. Тайм-аут и сбой языкового слияния имеют отдельные машинные коды и не оставляют непроверенный SCR.
- BlockParEditor 2.1 стал штатным: используется оригинальный EXE без DLL и legacy-manifest. Обнаруженная 1.9 остаётся поддержанной через локальный CP1251-совместимый EXE; обе версии проверяются семантическим циклом `TXT → DAT → TXT`.
- Регрессии охватывают UTF-8, CP1251 и UTF-16. `https://` и любое другое `//` внутри значения блокируется до BlockPar, поскольку проверенные 1.9 и 2.1 обрезают такую строку как комментарий.
...
В наборе из 281 теста используются нативные PNG/GI/QM/QMM-кодеки, RSM/rsmc и локальные BlockParEditor/RScript:
