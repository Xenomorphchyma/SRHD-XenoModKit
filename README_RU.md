...
- Сырые ссылки `Item`, полученные от `CreateQuestItem`, нельзя сохранять в
  общих `TVar` и массивах между ходами: runtime-lint требует сохранять
  `Id(item)` и восстанавливать объект через `IdToItem`.
- Межпроцедурный анализ отслеживает разгрузку через `GetItemFromShip` и
  блокирует `ShipOut` того же корабля в том же прямом пути. `OrderTakeOff` и
  фактический `ShipOut` должны выполняться в разные ходы.
- Прямой обход изменяемой группы проверяется на `ShipOut` текущего
  `GroupShip`: требуется обратный обход, коррекция индекса или завершение
  обработчика.
- Валидатор RSON проверяет, что `TItem` находится в `Items` и содержит строковое
  `+Place`. Необязательный `Items.Count` проверяется только при его наличии.
...
В 129 тестах используются нативные PNG/GI/QM/QMM-кодеки и локальные BlockParEditor/RScript:
