...
- Общие `TVar`, используемые как `Planet`, `Star` или `Ship`, проверяются на
  безопасное хранение между сохранениями: отдельный `Id(object)`, обнуление
  старой ссылки и доказанный восстановитель. Недоступный в игре голый вызов
  `IdToStar` блокируется; локальная ограниченная реализация через
  `GalaxyStars()`/`GalaxyStar(i)` распознаётся как безопасная.
- Локально доказуемые временные ссылки, заново присваиваемые перед каждым
  использованием, не дают ложного предупреждения.
- Уменьшение индекса больше не считается защитой прямого цикла
  `GroupCount`/`GroupShip` после `ShipOut`: изменение группы выносится в
  отдельный обратный проход, после которого обработчик завершается.
- `IdToShip` разрешён только при доказанном ID больше `1`. `ShipJoin` с
  подавленным начальным State нельзя сочетать с `OrderLock` без явного
  `ChangeState`, а `ShipInCurScript` не доказывает принадлежность нужной группе.
- Вызовы сверяются с реестром API SRHD 2.1.2500, локальными функциями и явными
  импортами. Неизвестное имя блокируется до RScript и игры.
- Для динамических массивов `newarray(1)`/`ArrayClear` проверяются служебный
  индекс `0`, правильная пустота `ArrayDim <= 1` и размеры парных persistent-массивов;
  фиксированные `newarray(N)` по-прежнему допускают индекс `0`.
- Добавлены межпроцедурные проверки `RndObject(..., Item)`, массового
  detach/unlink/free предметов, позднего hyperspace-guard, повторного чтения
  изменённой группы, сырой цели `ShipGetBad` и cleanup без границы хода.
- Непарный апостроф в `//` блокируется как подтверждённый риск `Not link var`;
  парные строковые литералы в полностью закомментированном коде допускаются.
- Для каждого этапа RScript действуют 60 секунд без подтверждённого прогресса и
  общий аварийный потолок 300 секунд. Явный положительный таймаут заменяет
  общий потолок, а `0` отключает оба ограничения.
...
блоки кода и добавленные/устранённые runtime-замечания, не сохраняя RSON мода.
...
По умолчанию каждый этап RScript получает скользящее 60-секундное окно без
подтверждённого прогресса и общий аварийный потолок 300 секунд. Изменение
ожидаемого файла, файловый ввод-вывод процесса или переход шага скрытой
автоматизации сдвигают короткое окно; простая загрузка CPU прогрессом не
считается. Положительный `--timeout`, `--decompile-timeout` или
`--roundtrip-timeout` заменяет общий потолок, а ноль отключает оба ограничения.
Статический preflight всё равно выполняется до запуска старого компилятора.
...
В наборе из 158 тестов используются нативные PNG/GI/QM/QMM-кодеки и локальные BlockParEditor/RScript:
...
Дополнительно `dev`-аудит обработал все 421 установленных мода: 3789 проверок, 0 падений
