...
`allow` из проекта, выбранного варианта и `project build --allow` применяется
также к runtime-preflight компиляции RSON. Подавленные сообщения остаются в
отчёте, а набор исключений входит в ключ кэша. `project plan` учитывает
настроенное в TOML `allow`; изменение этого списка требует повторной сборки.

Для исходников внутри папки мода кэш использует стабильный относительный путь
`mod:SOURCE/...` и SHA-256 содержимого после применения overlay. Случайное имя
временного staging не входит в ключ. Изменение исходника по-прежнему вызывает
cache miss, а неизменная повторная сборка и `project plan` используют один ключ.
