...
- Runtime-lint обнаруживает сохранённые в общих `TVar` сырые ссылки `Planet`, `Star` и `Ship`. Для долгоживущего состояния требуется отдельный числовой `Id(object)`, обнуление старой ссылки при миграции сохранения и восстановление через соответствующий `IdToPlanet`/`IdToStar`/`IdToShip`.
...
- На старой аварийной версии Coalition Supply Lines найдены обе причины падения; исправленные основной и тестовый RSON проходят строгий lint без замечаний. Локальный корпус RSON обработан без сбоев анализатора.
...
В версии 0.9.2 набор содержит 135 тестов; полный набор проверяет нативные PNG/GI,
