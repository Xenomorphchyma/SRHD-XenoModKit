...
- Добавлен собственный бинарный reader для QM 2/3/4 и QMM 6/7, не требующий TGE или GUI.
- Добавлены JSON-экспорт и детерминированная сборка QMM 7 с обязательной повторной загрузкой и смысловым сравнением.
- Аудит квестов проверяет граф переходов, параметры, условия, формулы, отсутствующие ссылки, недостижимые локации и потенциальные автоматические циклы.
- Релизный аудит выполняет `QM/QMM → QMM → модель`; старый QM безопасно обновляется до QMM 7.
- Реализация проверена на 48 установленных QMM и 18 независимых старых QM; все прошли смысловой round-trip.
...
В версии 0.9.0 проходит 121 тест; полный набор проверяет нативные PNG/GI,
