...
### Что изменилось в 0.9.6

- `script lint-runtime`, `script audit-mod` и release-аудит теперь проверяют всю цепочку локализации ответа: `TDialogAnswer → ключ RScript → Script/<ScriptName>/<key> → Lang.dat`.
- Отдельные ошибки сообщают об отсутствующем `Lang.dat`, узле скрипта, числовом ключе и пустой видимой подписи; диагностика содержит имя диалога, `#` ответа, `AMsg.Num`, ожидаемый путь и фактически проверенный файл.
- `script-dialog-lang-value-code-stub` блокирует непустые, но неигровые значения `DAnswer(...)`, `DText(...)`, `CT(...)` и `Format(...)` внутри `Script/<ScriptName>/<key>`: в Lang.dat должна лежать обычная отображаемая строка.
- Runtime-доказательством считается только `CFG/<язык>/Lang.dat`. `DATA/Script/Lang.dat` допустим как артефакт сборки/импорта RScript, но не подтверждает загрузку подписи игрой; одиночный такой DAT блокируется как `script-dialog-lang-runtime-dat-missing`.
- Статические `DAdd/TDialogAnswer` не запрещаются: при корректном языковом CFG это штатный поддерживаемый граф. Для динамического `InjectAnswer` отдельно проверяются целевой `TDialog` и достижимый обработчик.
- `runtime-shippicksitem-stale-after-forced-transfer` предупреждает, когда ручная цепочка `GetItemFromStar → AddItemToShip` оставляет включённым прежний `ShipPicksItem(..., 1)`: корабль может продолжить преследовать исчезнувшую добычу вместо следующего приказа.
- `script validate`, `script build` и release-аудит блокируют `TGroup` без исходящих связей к `TPlanet` и `TState`, не позволяя RScript 4.10f молча зависнуть в главном окне Build без SCR.
- Для релиза без RSON бинарный аудит SCR извлекает только доказанные ссылки `DAnswer(...CT("Script.<name>.<key>"))`; неподтверждённые номера не угадываются.
- Непустой соседний `*.lang.txt`, созданный RScript, больше не считается достаточным: `script-generated-lang-unpublished` показывает ключи, не попавшие в игровой DAT.
- `TDialogAnswer.Msg` с незавершённым выражением `DAnswer(...)` останавливается до запуска RScript как `rscript-dialog-answer-msg-missing-semicolon`. Обычный текст без вызова остаётся допустимым.
- Видимый литерал внутри `DAnswer('...~текст')` также блокируется до компиляции как `rscript-dialog-answer-msg-inline-text`; правильная форма использует `DAnswer(CT('Script.<name>.<key>'));`, а сам текст хранится в Lang.dat. Управляющий `DAnswer('exit');` остаётся допустимым.
- Отсутствие уже упакованного Lang.dat не блокирует preflight нового проекта: фрагмент RScript сначала проверяется во staging. Обычные строки можно собрать в DAT, а CT/code-заглушки требуют проверенный `--lang-base`.
...
Текущий набор из 261 теста проверяет нативные PNG/GI, лексический preflight,
