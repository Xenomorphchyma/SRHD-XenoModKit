...
### Что изменилось в 0.8.5

- Все скрытые RScript/BlockParEditor запускаются в Windows Job Object с `KILL_ON_JOB_CLOSE`.
- Аварийное завершение агента или Python теперь уничтожает редактор и всё его дерево процессов.
- Именованный межпроцессный mutex не позволяет нескольким агентам одновременно создавать скрытые редакторы.
- Добавлена headless-диагностика `srhd doctor processes` и безопасная очистка `--terminate`.
- Редакторы остаются видны во вкладке «Подробности» Диспетчера задач и допускают обычное ручное завершение.
- Штатный выход, таймаут, принудительная остановка родителя и параллельный запуск покрыты Windows-тестами.
...
В версии 0.8.5 проходит 113 тестов; полный набор проверяет нативные PNG/GI,
...
агента. На локальном корпусе глубоко проверено 3098 из 3131
