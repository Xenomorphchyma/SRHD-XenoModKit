...
### Что изменилось в 0.9.1

- Runtime-lint блокирует сохранение сырой ссылки `Item` в общих `TVar`/массивах; штатный шаблон — хранить `Id(item)` и восстанавливать объект через `IdToItem` на текущем ходу.
- Межпроцедурный анализ обнаруживает `GetItemFromShip`/разгрузку с последующим `ShipOut` того же корабля в одном прямом пути. Взлёт и удаление должны быть разделены границей хода.
- Прямой обход `GroupCount`/`GroupShip` теперь проверяется на удаление текущего корабля через `ShipOut` без обратного обхода или коррекции индекса.
- RSON preflight проверяет доказанную форму графовых `TItem`: коллекцию `Items`, строковое поле `+Place` и согласованность `Items.Count`, если этот необязательный счётчик сериализован.
- Правила проверены на исправленном основном и тестовом Coalition Supply Lines без ложных срабатываний; полный набор содержит 129 тестов.
...
В версии 0.9.1 проходит 129 тестов; полный набор проверяет нативные PNG/GI,
...
Он выполнил 3762 проверки и распознал 48 QM/QMM.
