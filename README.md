...
- анализировать, декомпилировать, изменять и собирать `RSON`, `SVR` и `SCR`;
...
- `script decompile` управляет декомпилятором RScript только на изолированном невидимом desktop, не изменяет исходный SCR и публикует RSON лишь после цикла `SCR → RSON → SCR`.
...
В версии 0.8.2 проходит 81 тест. Дополнительно dev-аудит был выполнен на 418 установленных модах без падений валидаторов.
