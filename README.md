...
- Runtime-lint обнаруживает не зарегистрированные в графе persistent-переменные и повторные локальные объявления, включая списки `var a,b`. Объектный вызов после `&&`/`||` больше не считается защищённым: RScript не гарантирует безопасное короткое замыкание, поэтому guard и вызов должны быть разнесены по операторам.
...
В версии 0.9.4 полный набор из 172 тестов проверяет нативные PNG/GI,
