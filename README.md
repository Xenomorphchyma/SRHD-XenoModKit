...
### Что изменилось в 0.9.8

- GAI-кодек теперь разбирает raw- и `ZL01(GI)`-кадры, штатные пустые позиции `(offset=0, size=0)` и прозрачные GI `0×0`. Поддержка подтверждена на всех 853 loose GAI установленных модов.
- Полностью пустая корректная анимация больше не считается повреждённой: аудит выдаёт неблокирующее `resource-empty-animation-placeholder`, чтобы автор подтвердил намеренное скрытие изображения.
- `resource extract` распаковывает ZL01-кадры в обычные GI, а пустые позиции сохраняет без потери номера; `build-gai` умеет собрать их обратно с шаблоном и auxiliary-блоком.
- Runtime-lint различает настоящую Global-фазу и выполняемый после `GRun` `Code.Type=Init`. Обращения к `Player()` в Init и официальный read-only bootstrap Global больше не дают ложных ошибок; неподтверждённая работа с миром в Global остаётся предупреждением.
- `compat` моделирует стабильную сортировку движка по возрастанию `Priority`: исходная позиция `CurrentMod` сохраняется как `configured_order`, рассчитанная — как `order`. Равные приоритеты не переставляются, отсутствие поля означает `0`, а рассинхрон списка выдаёт предупреждение без изменения `ModCFG.txt`.
- Runtime-lint находит фоновый пакетный `Buy*`, если незавершённая партия публикуется в общий `TVar`/`TGroup` и при повторном входе может быть обойдена с мутациями без раннего barrier. Одиночная настройка свежего корабля и пакет без такого частичного обхода остаются допустимыми.
- Runtime-lint предупреждает о `GetData` у корабля из `IdToShip` до доказательства завершённого взлёта/гиперперехода и стабильного размещения. Безопасный пользовательский predicate учитывается, а общий запрет `IdToShip` или обычного `GetData` не вводится.
- Отдельная межходовая проверка сопоставляет полный цикл `Buy*`/`TransferShip → persistent ID → IdToShip → переход → GetData`. Spatial-guards не считаются доказательством сохранности внутреннего script-data после перехода; обычные обращения без полного lifecycle не затрагиваются.
