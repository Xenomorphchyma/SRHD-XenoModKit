...
- `script build --json` возвращает схему `srhd-modkit-script-build-v1` и при успехе, и при неуспехе. Поля `preflight_passed`, `compiler_started`, `compiler_output_created` и `published_outputs` разделяют статическую проверку, фактический запуск RScript и публикацию файлов. Подтверждённое возвращение RScript к главному окну Build без SCR и прогресса получает машинный код `rscript-build-silent-main-window-stall`, а не безымянный тайм-аут.
...
- `script compare-scr` включает сравнение persistent-схемы, смысловую карту диалогов и необязательные риски обновления SAV для изменившегося SCR с прежним runtime-именем. Полный набор из 172 тестов проверяет новые правила и прежние форматы.
